...
export default class ScrollAnimateItem extends Component {
  ...
  UNSAFE_componentWillReceiveProps({ scrolltop }) {
    ...
    if (currentIndex < startIndex) this.topValue = startIndex;
    ...
    } else this.topValue = endIndex;

    this.el.style.top = this.topValue + "px";
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div
          style={{ height: "100%", position: "absolute", width: '100%', ...style }}
          {...props}
          ref={el => (this.el = el)}
        >
          {children &&
